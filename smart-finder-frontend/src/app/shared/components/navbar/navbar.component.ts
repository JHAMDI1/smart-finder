  ...
  template: `
      ...
      <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          ...
          <ng-container *ngIf="isAdmin()">
            <a routerLink="/admin/dashboard" routerLinkActive="text-indigo-600 font-semibold" class="hidden md:flex items-center gap-2 text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-lg font-medium transition-colors border border-indigo-200">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <nav class="md:hidden fixed bottom-6 left-4 right-4 z-50">
      <div class="bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl shadow-gray-900/40 p-2 flex justify-around items-center border border-gray-800">
        ...
        <!-- Espace Pro -->
        <a *ngIf="isOwner()" routerLink="/owner/dashboard" routerLinkActive="bg-amber-500/20 text-amber-400" class="flex flex-col items-center justify-center w-16 h-14 rounded-xl text-gray-400 hover:text-white transition-all">
          <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Admin -->
        <a *ngIf="isAdmin()" routerLink="/admin/dashboard" routerLinkActive="bg-indigo-500/20 text-indigo-400" class="flex flex-col items-center justify-center w-16 h-14 rounded-xl text-gray-400 hover:text-white transition-all">
          <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
          </svg>
